...
function initBotPage() {
  ...
  btn.addEventListener('click', () => {
    ...
    const outputEl = document.getElementById('botOutput');

    const text = (inputEl.value || '').toLowerCase().trim();

    if (!text) {
      outputEl.textContent = "Сұрақ жаз 😄";
      return;
    }
    ...
    // 🔬 ДНҚ
    if (text.includes('днқ')) {
      answer = `
ДНҚ — генетикалық ақпарат тасымалдаушы молекула.

Негізгі құрылымы:
• Мономері — нуклеотид
• Құрамы:
  - азотты негіз (A, T, G, C)
  - дезоксирибоза
  - фосфат
...
РНҚ → рибоза
    ...
    // 🧬 РНҚ
      ...
      answer = `
РНҚ — ақуыз синтезіне қатысады.

Түрлері:
• аРНҚ — ақпарат тасиды
• тРНҚ — аминқышқыл тасиды
• рРНҚ — рибосома құрайды

Ерекшелігі:
• Бір тізбекті
• Тимин жоқ → орнына урацил

ҰБТ:
ДНҚ ≠ РНҚ айырмашылығын жиі сұрайды
      `;
    }

    // ❤️ ЖҮРЕК
    else if (text.includes('жүрек')) {
      answer = `
Жүрек — 4 камерадан тұрады:
...
Маңызды:
• Артерия → жүректен шығады
• Вена → жүрекке келеді

ҰБТ trap:
Артерия ≠ әрқашан оттегі
(өкпе артериясы — оттегісіз!)
      `;
    }
    ...
    // 🧠 МИТОЗ
      ...
      answer = `
Митоз — жасушаның бөліну процесі.
...
Нәтиже:
→ 2 бірдей диплоидты жасуша

ҰБТ:
митоз = өсу + регенерация
      `;
    }

    // 🌱 ФОТОСИНТЕЗ
    else if (text.includes('фотосинтез')) {
      answer = `
Фотосинтез:
...
өсімдік = O₂ береді
    ...
    // 🧪 НӘРУЫЗ
    else if (text.includes('нәруыз') || text.includes('белок')) {
      answer = `
Нәруыз — аминқышқылдардан тұрады.
...
Қауіп:
→ антидене түзіледі
→ ұрық эритроциттері бұзылады

ҰБТ:
2-ші жүктілік қауіпті
      `;
    }

    // 🌬 ТЫНЫС АЛУ
    else if (text.includes('тыныс')) {
      answer = `
Тыныс алу жүйесі:
...
өсімдік "терлейді"
    ...
    // 🤖 DEFAULT
      ...
      answer = `
Мен биология бойынша көмектесем 😄
...
Қысқа әрі ҰБТ стилінде түсіндірем.
      `;
    }
